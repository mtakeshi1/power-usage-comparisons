...
class SkunkDataAccess(val host: String, val user: String, val pwd: String, val database: String) extends DataAccess {
  ...
  def init() = {
    ...
    pool.use {session => session.use { s =>
      IO.pure(1)
    }}
  }

  val listProductsQuery: Query[Void, ProductInformation] = sql"select id,name from product".query(int4 ~ varchar).gmap[ProductInformation]

  val productById:Query[Int, Models.Product] = sql"select id, name, description, price from product where id = $int4".query(int4 ~ varchar ~ varchar ~ float8).gmap[Models.Product]
...
//    val insertEntryCmd: Command[List[Int ~ Int ~ Int]] =
  ...
  override def newOrder(items: List[Models.ShoppingCartEntry]): IO[Models.ShoppingCart] = {
    pool.use {_.use {session => session.transaction.use { tx =>
        for {
